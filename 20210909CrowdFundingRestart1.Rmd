...
Data from Kickstarter shows that over half of the projects listed are not successful. This makes the study of factors that contribute to success very relevant. Crowdfunding platforms are used for a host of reasons. On Kickstarter, as per our data, there are over 15 categories ranging from Dance to Technology. A number of small initiatives and startups raise money through this route. 
...
There is also a strong element of reciprocity. (Cite)
...
Table \@ref(tab:OverallSummary) and \@ref(tab:datasummary1) gives a summary of the data.
...
In Figure \@ref(alldate1) we see that while the number of new projects launched everyday varies to a large extent, it has increased since launch.

```{r alldate1,fig.cap="\\label{fig:alldate1}Number of projects launched by date",echo=FALSE, message=FALSE, warning=FALSE}
...
```{r alldate2,fig.cap="\\label{fig:alldate2}Proportion of successful projects by date",echo=FALSE, message=FALSE, warning=FALSE}
...
We look at month wise success ratios by dividing the total number of successful projects by the total number of projects launched in a particular month. In fig \@ref(monthwisesuccess1) we look at the ratio of successful projects for each month. We see that the overall ratio for any month is less than 0.5. This study gains importance in light of this information. We see that October has the lowest ratio of successful projects. We filter the dataset by currency and replicate the results for each currency and see a very similar pattern for USD nd other currencies. October is the lowest for a number of currencies. The number of bars might not be 12 in some currencies because of the distribution of data.

```{r monthwisesuccess1, fig.cap="\\label{fig:monthwisesuccess1}Proportion of Successful Projects by Month",echo=FALSE, message=FALSE, warning=FALSE}
library(ggplot2)
...
```{r daywise1,fig.cap="\\label{fig:daywise1}Proportion of Successful to Unsuccessful Projects by Day", echo=FALSE}
...
In Figure \@ref(fig:daywise1)we plot the proportion of successful projects. We see that Sundays are especially better. We replicate this chart for different currencies and find that except DKK representing Danish Kroner, Sunday is the best day to launch a project across jurisdictions. In Netherlands, Saturdays seem to be the best. 
...
Something\footnote{See if you want to do overachievement etc.}

\subsubsection{Words}

findFreqTerms(course_dtm,10000) [1] "game" "help" "make" "new" "will" 
findFreqTerms(course_dtm,5000) [1] "album" "art" "book" "bring" "can" "creat" "design" "film" "first" "game" "get" "help" "life" "love" "make" "music" "need" "new"\
...
knitr::kable(cortable) #See if this works
```

In Table \@ref(tab:RegTable11) we list the effect of each word based on a regression run on the success of the project while controlling for the day of week, month of the year, project duration and the total amount to be raised. 
In Table \@ref(tab:RegTable21) we list the effect of each word based on a regression run on the overachievement of the project while controlling for the day of week, month of the year, project duration and the total amount to be raised. 
```{r regressionsuccesstry1, echo=FALSE, message=FALSE, warning=FALSE, paged.print=TRUE, results='asis'}
wordlist<-c("baby","mother","ma ","mom ","father","pa ","papa","dad","parent","close ","friend","brother","sister","sibling","grandparent","grandchild","son","daughter","love","please","happy","thank","story","save","child","every","together","we"," us "," our ","mine","never","dead","die ","infect","pain","suffer","cancer","disease","amputee","enviro","homosexual","gay","fear","overcome","dear","most","best","worst","est ","smart","techn","ease","new","convenience","prevent","hunger","accessible","support")
...
knitr::kable(regtable22,caption = "This table shows the results for regression of the words on Overachievement and their p-values for words that do not have a statistically significant effect.")
```


```{r regressionsuccess, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE, results='asis'}
#c1$SuperCategory<-c1$Super_Category
c1$Super_Category<-ifelse(c1$Super_Category=="Film & Video", "Film and Video",c1$Super_Category)
...
library(stargazer)
stargazer(reg1,reg2,reg3, title="Regression results for use of particular words and success.",header=FALSE,
         omit=c("month","day","Super_Category"),
         omit.labels=c("Month","Day","Super Category") 
        )

stargazer(reg4,reg5,reg6, title="Regression results for use of particular words and success.",header=FALSE,
         omit=c("month","day","Super_Category"),
         omit.labels=c("Month","Day","Super Category") 
        )

stargazer(reg7,reg8,reg9, title="Regression results for use of particular words and success.",header=FALSE, 
         omit=c("month","day","Super_Category"),
         omit.labels=c("Month","Day","Super Category") 
        )

stargazer(reg10,reg11,reg12, title="Regression results for use of particular words and success.",header=FALSE, 
         omit=c("month","day","Super_Category"),
         omit.labels=c("Month","Day","Super Category") 
        )

stargazer(reg13,reg14,reg15, title="Regression results for use of particular words and success.",header=FALSE, 
         omit=c("month","day","Super_Category"),
         omit.labels=c("Month","Day","Super Category") 
        )
...
```{r regressionover, echo=FALSE, message=FALSE, warning=FALSE,results='asis'}
reg1<-glm(c1$over~c1$ease+c1$lngoal+c1$totalduration +c1$month+c1$day , family = 'binomial')
reg2<-glm(c1$over~c1$father+c1$lngoal+c1$totalduration +c1$month+c1$day , family = 'binomial')
reg3<-glm(c1$over~c1$please+c1$lngoal+c1$totalduration +c1$month+c1$day , family = 'binomial')
reg4<-glm(c1$over~c1$thank+c1$lngoal+c1$totalduration +c1$month+c1$day , family = 'binomial')
reg5<-glm(c1$over~c1$every+c1$lngoal+c1$totalduration +c1$month+c1$day , family = 'binomial')
reg6<-glm(c1$over~c1$never+c1$lngoal+c1$totalduration +c1$month+c1$day , family = 'binomial')
reg7<-glm(c1$over~c1$best+c1$lngoal+c1$totalduration +c1$month+c1$day , family = 'binomial')
reg8<-glm(c1$over~c1$est+c1$lngoal+c1$totalduration +c1$month+c1$day , family = 'binomial')
reg9<-glm(c1$over~c1$child+c1$lngoal+c1$totalduration +c1$month+c1$day , family = 'binomial')
reg10<-glm(c1$over~c1$dream+c1$lngoal+c1$totalduration +c1$month+c1$day , family = 'binomial')
reg11<-glm(c1$over~c1$ma+c1$lngoal+c1$totalduration +c1$month+c1$day , family = 'binomial')
reg12<-glm(c1$over~c1$dad+c1$lngoal+c1$totalduration +c1$month+c1$day , family = 'binomial')
reg13<-glm(c1$over~c1$poor+c1$lngoal+c1$totalduration +c1$month+c1$day , family = 'binomial')
reg14<-glm(c1$over~c1$support+ c1$lngoal+c1$totalduration +c1$month+c1$day , family = 'binomial')
reg15<-glm(c1$over~c1$forgive+c1$lngoal+c1$totalduration +c1$month+c1$day , family = 'binomial')
library(stargazer)
stargazer(reg1,reg2,reg3, title="Regression results for use of particular words and over-funding.",header=FALSE, 
         omit=c("month","day","Super_Category"),
         omit.labels=c("Month","Day","Super Category") 
        )

stargazer(reg4,reg5,reg6, title="Regression results for use of particular words and over-funding.",header=FALSE, 
         omit=c("month","day","Super_Category"),
         omit.labels=c("Month","Day","Super Category") 
        )

stargazer(reg7,reg8,reg9, title="Regression results for use of particular words and over-funding.",header=FALSE, 
         omit=c("month","day","Super_Category"),
         omit.labels=c("Month","Day","Super Category") 
        )

stargazer(reg10,reg11,reg12, title="Regression results for use of particular words and over-funding.",header=FALSE, 
         omit=c("month","day","Super_Category"),
         omit.labels=c("Month","Day","Super Category") 
        )

stargazer(reg13,reg14,reg15, title="Regression results for use of particular words and over-funding.",header=FALSE, 
         omit=c("month","day","Super_Category"),
         omit.labels=c("Month","Day","Super Category") 
        )
```
