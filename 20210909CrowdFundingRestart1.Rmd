...
c1<-mutate(c1, ma=(grepl("ma",c1$Blurb,1,0)+0))
...
```{r monthwise1, echo=FALSE, fig.height=5.5, fig.width=4, message=FALSE, warning=FALSE}
library(ggplot2)
...
```{r monthprop1, echo=FALSE, fig.height=5.5, fig.width=4, message=FALSE, warning=FALSE}
...
While more than 97 percent projects launched in march are successful, only 59 percent projects are successful in September.
...
```{r Readability1, message=FALSE, warning=FALSE}
library(quanteda)
...
```{r wordcorrelation, message=FALSE, warning=FALSE}
cor.test(c1$ma,c1$success)#PRetty Cool
cor.test(c1$mom,c1$success)#Negative Correlation
cor.test(c1$mother,c1$success)#No Correlation
cor.test(c1$cancer,c1$success)#Negative Correlation
cor.test(c1$kids,c1$success)#
...
SignificanRelations<-as.data.frame(t(SignificanRelations))
colnames(SignificanRelations)<-"pValue of Correlation"

stargazer(SignificanRelations,summary = FALSE)
```
